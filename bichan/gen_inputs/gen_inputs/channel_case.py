...
def roms_zgrid(num_layers=50):
    """
    Generate a ROMS vertical z-grid with specified number of layers.

    Parameters:
        num_layers (int): Number of vertical layers (50, 75, or 100)

    Returns:
        np.ndarray: Array of depths from 0 to -50 with num_layers+1 levels
    """
    if num_layers not in [50, 75, 100]:
        raise ValueError("num_layers must be 50, 75, or 100")

    if num_layers == 50:
        # Return the hard-coded reference levels (51 values)
        return np.array([
             0.0,
        ...
        return np.linspace(0, -50, num_layers + 1, dtype=np.float64)
...
def build_init(args, mesh):
    ...
    init["ssh"] = (("Time", "nCells"), np.zeros((1, mesh.xCell.size)))
    init["ssh"][0, :] = ssh


    # Get z-levels
    zlev_raw = -roms_zgrid(num_layers=args.num_layer)  # positive down
    zref_raw = zlev_raw.copy()

    # Set bottom depth from deepest z-level
    args.bot_depth = zlev_raw[-1]
    init["bottomDepth"] = args.bot_depth * xarray.ones_like(xcel)
    init["bottomDepthObserved"] = args.bot_depth * xarray.ones_like(xcel)

    # Expand z-levels over all cells
    zlev = np.tile(zlev_raw.reshape(-1, 1), (1, ssh.size))
    zref = np.tile(zref_raw.reshape(-1, 1), (1, ssh.size))
    ...
    init["refTopDepth"] = (("nVertLevels"), np.zeros(args.num_layer))

    init["refZMid"] = (("nVertLevels"), np.zeros(args.num_layer))
    init["refBottomDepth"] = (("nVertLevels"), np.zeros(args.num_layer))
    init["refInterfaces"] = (("nVertLevelsP1"), np.zeros(args.num_layer + 1))
    init["vertCoordMovementWeights"] = (("nVertLevels"), np.ones(args.num_layer))
    ...
    init["restingThickness"] = (
        ("nCells", "nVertLevels"),
        np.zeros((xcel.size, args.num_layer))
    )
    init["layerThickness"] = (
    ...
    temp = args.temperature0 + args.n_squared / args.alpha / GRAV * RHO0 * zmid

    print("trc. T-min. =", temp.min().values)
    print("trc. T-max. =", temp.max().values)

    ymid = 0. * zmid
    for k in range(args.num_layer):
        ymid[:, k] = ycel.values

    half = 0.5 * (ycel.min().values + ycel.max().values)

    salt = args.salinity0 + args.m_squared / args.beta / GRAV * RHO0 * (ymid - half)
    ...
    for iEdge in range(mesh.xEdge.size):
        ...
        pert = PERT * (
            np.sin(2. * np.pi * (mesh.xEdge.values[iEdge]) / 100.E+03) *
            np.sin(2. * np.pi * (mesh.yEdge.values[iEdge] - ymin) / 100.E+03)
        )
