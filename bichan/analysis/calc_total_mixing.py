...
- Mtot_salt = 1/V iiint (chiSpurSalt + 2*kappa_v * (ds/dz)**2) dz dA 
...
resolutions = [10000, 9000, 8000, 7000, 6000, 5000, 4000, 3000, 2000, 1000]  # meters
layers = [5, 10, 50, 100]
...
for res_key, zdict in datasets.items():
    ...
    for z_key, ds in zdict.items():
        ...
        # Output file paths
        tot_path = os.path.join(output_dir, f"mtot_salt_vavg_{res_key}_{z_key}.npy")
        ...
        # Numerical mixing
        mtot_salt_dv = (1/V)*((
            (dsd.chiSpurSaltBR08 + dsd.chiPhyVerSalt) *
            layer_thickness *
            area_cell
        ).sum(dim=['nVertLevels', 'nCells']))

        # Save results
        np.save(tot_path, mtot_salt_dv.values)
